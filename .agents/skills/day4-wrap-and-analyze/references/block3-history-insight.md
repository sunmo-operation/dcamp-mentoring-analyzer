...
- 파일 형식: `.jsonl`이라는 형식인데, 대화 내용을 한 줄씩 기록한 텍스트 파일입니다 (이 형식을 외울 필요는 없습니다. Claude가 알아서 읽어줍니다)
- 프로젝트별로 폴더가 나뉘어 있음
- 하나의 세션(대화) = 하나의 파일
...
**숫자로 보면:**
- 12MB 세션 파일 -> 약 160KB로 압축
- 파일 변경 이력, 큐 작업 같은 부속 자료가 전체의 94%
- 실제 사용자와 Claude의 대화는 약 **6%만** 차지

비유하자면, 12MB짜리 일기장에서 실제로 쓴 글은 160KB뿐이고, 나머지는 날짜 도장, 날씨 기록, 영수증 같은 부속 자료인 셈입니다. history-insight는 이 부속 자료를 다 걷어내고, **실제 글(대화)만 모아서** 분석합니다.
...
Block 2에서 사용했던 **작업 세션 탭** (또는 새 탭)에서 실습합니다. 교육 세션 탭이 아닌 별도 탭에서 진행하세요.
...
| "jq가 필요합니다" | Mac: `brew install jq` / Windows: `winget install jqlang.jq` / 모르겠으면 강사에게 문의하세요. |
...
정답: 1번. 세션 파일의 94%는 파일 변경 이력, 큐 작업 같은 부속 자료다. 실제 사용자와 Claude의 대화는 약 6%만 차지한다. history-insight는 이 6%만 추출해서 분석하므로, 12MB 파일도 ~160KB로 압축된다.
