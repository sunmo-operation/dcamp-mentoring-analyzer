...
Step 3: 재료 준비             -> Git 확인 (오늘 뭘 했는지 파악)
...
### Step 3: Git 상태 확인 섹션 추가

재료를 확인하는 단계입니다. "오늘 이 세션에서 뭘 했는지"를 파악합니다.

```
SKILL.md에 Step 1: Git 상태 확인 섹션을 추가해줘.
"오늘 이 세션에서 어떤 파일을 만들거나 수정했는지" 파악하는 부분이야.
git status와 git diff 명령어를 사용해서 변경 내용을 요약하는 구조로 만들어줘.
```

> 이 부분은 "오늘 뭘 했는지" 확인하는 단계입니다. Claude가 Git이라는 도구로 변경 사항을 자동으로 확인합니다. 여러분이 Git 명령어를 직접 칠 필요는 없습니다. 스킬이 실행되면 Claude가 알아서 처리합니다.
...
각 에이전트가 subagent(Task)로 동시에 실행되도록 작성해줘.
...
> - **learning-extractor**: "오늘 배운 것을 TIL(Today I Learned)로 정리"
...
완성된 SKILL.md가 6개 섹션(frontmatter, 다이어그램, Git 확인, Phase 1, Phase 2, 결과 통합)을 모두 갖고 있는지 확인합니다.
