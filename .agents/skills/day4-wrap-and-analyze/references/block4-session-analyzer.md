...
| 3. 디버그 로그 분석 | 실제 동작 추출 | CCTV 녹화본 재생 |
...
session-analyzer에게 "어떤 세션을 분석할지" 알려줘야 합니다. Block 2에서 사용했던 작업 세션 탭이 아직 열려 있으면 그 탭에서, 닫았다면 새 탭을 열고 `claude`를 실행한 뒤 아래를 입력합니다:

```
가장 최근에 /my-session-wrap을 실행한 세션을 분석해줘
```

> Claude가 자동으로 해당 세션을 찾아서 분석합니다. 세션 ID(`3cc71c9f-d27a-4233-...` 같은 긴 문자열)를 직접 찾을 필요는 없습니다.

> **세션을 못 찾는 경우**: "세션을 찾을 수 없다"고 나오면, "현재 프로젝트의 가장 최근 세션을 분석해줘"라고 범위를 넓혀보세요. 그래도 안 되면 "~/.claude/projects/ 에서 이 프로젝트의 최근 세션 파일을 찾아서 분석해줘"라고 구체적으로 요청하세요.

### 2단계: session-analyzer 실행
...
```
방금 실행한 세션을 my-session-wrap 스킬의 SKILL.md와 비교 분석해줘
```
