...
[분석 프레임워크]
① PMF 렌즈: 고객 데이터 기반 vs 팀 가설(뇌피셜) 구분. "만들고 싶은 것"과 "돈 내고 살 것"의 갭 지적.
② 방 안의 코끼리: 반복 언급되지만 담당자/액션 없이 이월되는 이슈. 팀이 피하는 불편한 진실.
③ 싱크홀: 서로 다른 소스(멘토링/KPT/전문가요청) 간 정보 비대칭 지적.
...
- 금액은 반드시 억원 단위로 표기. 100,000,000원=1억원, 50,000,000원=0.5억원, 1,700,000,000원=17억원. "원" 단위 그대로 쓰지 말 것.

[서술 규칙]
- 문체: 모든 문장은 ~함, ~중임, ~예정임, ~필요함 등 보고서체. 감정 표현·대화투·불필요한 주어 제거.
- 수치: "많음/적음/증가" 금지. 매출·목표·금액은 반드시 구체적 숫자로 표기. 수치가 없으면 "데이터 없음" 명시.
- 깊이: 단순 나열 금지. 논의 배경·원인·수치·진행상황·멘토 조언을 포함. 한 항목당 2~3개 핵심 사실 이상.
...

{
  "executiveSummary": {
    ...
    "reportBody": "핵심 지표와 현황을 줄바꿈(\\n)으로 구분하여 5~7건 나열. 매출·MAU·전환율 등 정량 수치와 현재 실행 상황을 모두 포함. 형식: 지표명: 수치 또는 상태. 금액은 반드시 억원 단위로 표기(예: 1.7억원, 50억원)",
