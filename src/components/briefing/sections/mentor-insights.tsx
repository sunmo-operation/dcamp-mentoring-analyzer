...
export function MentorInsights({
  ...
  const safeRepeated = Array.isArray(repeatedAdviceLines) ? repeatedAdviceLines : [];
  const safeIgnored = Array.isArray(ignoredAdviceLines) ? ignoredAdviceLines : [];
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        {/* 반복 강조 조언 — 간결한 줄 기반 */}
        {safeRepeated.length > 0 && (
          <div>
            <p className="text-xs font-semibold text-muted-foreground mb-2">반복 강조 조언</p>
            <div className="space-y-1.5">
              {safeRepeated.slice(0, 4).map((line, i) => (
                <div key={i} className="border-l-2 border-primary/40 pl-3">
                  <p className="text-sm leading-relaxed">{safeStr(line)}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {safeIgnored.length > 0 && (
          <div className="rounded-lg border border-red-200 bg-red-50/50 p-3 dark:border-red-800 dark:bg-red-950/50">
            ...
            <div className="space-y-1.5">
              {safeIgnored.slice(0, 3).map((line, i) => (
                <div key={i} className="border-l-2 border-red-300 dark:border-red-700 pl-3">
                  <p className="text-sm text-red-700 dark:text-red-300 leading-relaxed">{safeStr(line)}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {executedAdvice && (
          <>
            ...
            <div>
              ...
              <p className="text-sm leading-relaxed">{safeStr(executedAdvice)}</p>
            </div>
          </>
        )}
      </CardContent>
